...
for ss = 1:nSFs
    for cc = 1:nTestingChannels
        ...
        whichChannelPrimary1 = testingChannels(cc);
        whichChannelPrimary2 = testingChannels(cc);
        whichChannelPrimary3 = testingChannels(cc);
        ...
        %
        % Set spatial frequency.
        targetCyclesPerDeg = targetCyclesPerDegs(ss);
        ...
        if or(nSFs > 1, nTestingChannels > 1)
            if ~and(ss == nSFs, cc == nTestingChannels)
                fprintf('Press any key to display image \n',whichChannelPrimary1,targetCyclesPerDeg);
                pause;
            end
        end
        if cc == nTestingChannels
