function [screenPrimaryChannelObject,backgroundChannelObject] = SetupChannelPrimariesMel(colorDirectionParams,channelCalObjs,projectIndices,options)
% Set up channel primaries that reproduce desired cone contrasts.
%
% Syntax:
%    [screenPrimaryChannelObj,bgChannelObject] = SetupChannelPrimaries<e;(colorDirectionParams,channelCalObjs,projectIndices)
%
% Description:
...
% Get background for all primaries.
for pp = 1:nScreenPrimaries
    channelBackgroundPrimaries(:,pp) = targetBackgroundPrimaryVal*ones(nChannels,1);
    channelBackgroundSpd(:,pp) = PrimaryToSpd(channelCalObjs{pp},channelBackgroundPrimaries(:,pp));
    channelBackgroundXYZ(:,pp) = colorDirectionParams.T_xyz*channelBackgroundSpd(:,pp);
end
...
for pp = 1:nScreenPrimaries
    ...
    [screenPrimaryPrimaries(:,pp),screenPrimaryPrimariesQuantized(:,pp),screenPrimarySpd(:,pp),screenPrimaryContrast(:,pp),screenPrimaryModulationPrimaries(:,pp)] ...
        ...
        colorDirectionParams.T_receptors, channelCalObjs{pp}, colorDirectionParams.B_natural{pp}, projectIndices, colorDirectionParams.primaryHeadroom,...
