...
for pp = 1:nPrimaries
    regressProjectorPrimaries(:,pp) = P_device{pp}\theCheckData.isolatingSpdMeasured(:,pp);

    figure(primaryValueFig);
    subplot(1,nPrimaries,pp); hold on;
    plot(checkTargetPrimaries(:,pp),regressProjectorPrimaries(:,pp),'ro','MarkerSize',12,'MarkerFaceColor','r');
    ...
    subplot(nPrimaries,1,pp); hold on;
    plot(wls,P_device{pp}*regressProjectorPrimaries(:,pp),'g','LineWidth',2);
