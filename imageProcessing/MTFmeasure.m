...
image = imread('Pixel100_defocused_5.tiff','tiff');
...
imagecrop_25 = imagecrop(0.25*Ypixel_crop,:);
imagecrop_50 = imagecrop(0.50*Ypixel_crop,:);
imagecrop_75 = imagecrop(0.75*Ypixel_crop,:);
...
%% 2) Modulation Contrast measure
% contrast = (max()-min())/(max()+min()); % Ideal if as close as to 1, so
% the bigger number, the better contrast